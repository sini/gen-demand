...
| Collecting / fanning in the demand list — gen-demand took an already-ordered list | **`gen-view`, which inherited it — `gen-pipe` RETIRED as a library rather than moving as one.** ADR-0010 §3, the same shape of ruling that retired this library: twelve of gen-pipe's seventeen exports name gen-view constructs, `sel` retires into `gen-select`, and `join` — the fan-in this row points at — has no single successor construct and is a caller-side composition. Off the `gen/lib/mkGenLibs.nix` roster and not a `gen` hub input; the repository orphans as reference under ADR-0031 §3's F3 pattern |
| Turning `resources`/`wiring` into edges, modules, or configuration | **`gen-view`, which inherited it — `gen-edge` RETIRED as a library rather than moving as one.** ADR-0010 §3 retires the content-movement contract into the movement vocabulary; the `(S,T,P,M)` algebra, edge-set derivation and Kahn-ordered materialization each name a gen-view construct (the fourth destination §3 gained on 2026-08-20). Off the `gen/lib/mkGenLibs.nix` roster and not a `gen` hub input; the repository orphans as reference under ADR-0031 §3's F3 pattern. ★ Both rows on this sheet now point at retirements, which is the honest state: this library retired first, and the two siblings its rows deferred to have since followed |
